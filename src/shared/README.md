...
Do not read `config/env` or `import.meta.env` from shared code. If a shared utility needs environment-derived values, accept them as parameters or factory options.

不要在 shared 代码中读取 `config/env` 或 `import.meta.env`。如果 shared 工具需要环境派生值，应通过参数或工厂选项传入。
