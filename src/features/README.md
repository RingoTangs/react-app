...
Features 可以依赖 `shared` 或其他公共 feature 暴露的稳定公共 API。对于 theme、auth 或 i18n 这类 provider 支撑的能力，应从 `shared/<capability>` 或 `features/<domain>` 导入 hooks 和 types，不要从应用启动模块导入。
